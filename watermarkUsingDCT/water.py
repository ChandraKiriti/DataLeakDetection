...
def watermarkImage(img):
    global image
    global watermark
    image = os.path.join(path,'input\\img.jpg')
    watermark = os.path.join(path,'qrcode\\test.png')
   
    model = 'haar'
    level = 1
    imageArray = convertImage(image, 2048,"img.jpg")
    watermarkArray = convertImage(watermark, 128,"test.png")
